...
function vod(dingdantype,userid,orderid){
  ...
  area : ['750px' , '600px'],
  content: 'express.php?userid='+userid+'&orderid='+orderid,
  });
  }
}
...
<form name="form" id="form" method="post" action="order_save.php">
	<table width="100%" border="0" cellpadding="0" cellspacing="0" class="dataTable">
		...
		if($check!=""){
		      if($check=='pay'){ //待提取
		      $dopage->GetPage("SELECT * FROM `orderform` where State='1'",15);	
			    }elseif($check=='unpay'){ //待付款
		      $dopage->GetPage("SELECT * FROM `orderform` where State='5'",15);	
			    }elseif($check=='tiqu'){  //以提取
			$dopage->GetPage("SELECT * FROM `orderform` where State='8'",15);	
				}elseif($check=='yuyue'){  //预约订单
			$dopage->GetPage("SELECT * FROM `orderform` where address is not null",15);	
				}elseif($check=='ziti'){  //自提订单
			$dopage->GetPage("SELECT * FROM `orderform` where dingdantype=0",15);	
				}elseif($check=='kd'){  //快递订单
			$dopage->GetPage("SELECT * FROM `orderform` where dingdantype=2",15);	
				}elseif($check=='jiuqian'){  //酒钱支付
			$dopage->GetPage("SELECT * FROM `orderform` where  PaymentType=1",15);	
				}elseif($check=='jinqian'){  //金钱支付
			$dopage->GetPage("SELECT * FROM `orderform` where  PaymentType=2",15);	
				}elseif($check=='hunhe'){  //混合支付
			$dopage->GetPage("SELECT * FROM `orderform` where  PaymentType=3",15);	
				}elseif($check=='wei'){  //未支付
			$dopage->GetPage("SELECT * FROM `orderform` where  PaymentType=4",15);	
				}elseif($check=='today'){  //今日订单
			$time=date("Y-m-d");
			$dopage->GetPage("select * from orderform where CreatTime like '%$time%' and (State=1 or State=8)",15);
				}elseif($check=='tomorrowdingdan'){  //昨日订单
			$time=date("Y-m-d",strtotime("-1 day"));
			$dopage->GetPage("select * from orderform where CreatTime like '%$time%' and State=1",15);
				}elseif($check=='todaytiqu'){  //今日提货
			$time=date("Y-m-d");
			$dopage->GetPage("select * from orderform where TakeTime like '%$time%' and State=8",15);
				}elseif($check=='tomorrowtiqu'){  //今日提货
			$time=date("Y-m-d",strtotime("-1 day"));
			$dopage->GetPage("select * from orderform where TakeTime like '%$time%' and State=8",15);
				}
		  }elseif($keyword!=""){
		  $dopage->GetPage("SELECT * FROM `orderform` where OrderId like '%$keyword%' or tiquma like '%$keyword%' or UserId like '%$keyword%'",15);		
		  }else{
		     $dopage->GetPage("SELECT * FROM `orderform`",15);	
		  }
	      }elseif($_SESSION['adminlevel']==2){
			if($check!=""){
		      if($check=='pay'){ //待提取
		      $dopage->GetPage("SELECT * FROM `orderform` where State='1' and city='$live_city'",15);	
			    }elseif($check=='unpay'){ //待付款
		      $dopage->GetPage("SELECT * FROM `orderform` where State='5' and city='$live_city'",15);	
			    }elseif($check=='tiqu'){  //以提取
			$dopage->GetPage("SELECT * FROM `orderform` where State='8' and city='$live_city'",15);	
				}elseif($check=='yuyue'){  //预约订单
			$dopage->GetPage("SELECT * FROM `orderform` where address is not null and city='$live_city'",15);	
				}elseif($check=='ziti'){  //自提订单
			$dopage->GetPage("SELECT * FROM `orderform` where address is null and city='$live_city'",15);	
				}elseif($check=='jiuqian'){  //酒钱支付
			$dopage->GetPage("SELECT * FROM `orderform` where  PaymentType=1 and city='$live_city'",15);	
				}elseif($check=='jinqian'){  //金钱支付
			$dopage->GetPage("SELECT * FROM `orderform` where  PaymentType=2 and city='$live_city'",15);	
				}elseif($check=='hunhe'){  //混合支付
			$dopage->GetPage("SELECT * FROM `orderform` where  PaymentType=3 and city='$live_city'",15);	
				}elseif($check=='wei'){  //未支付
			$dopage->GetPage("SELECT * FROM `orderform` where  PaymentType=4 and city='$live_city'",15);	
				}elseif($check=='today'){  //今日订单
			$time=date("Y-m-d");
			$dopage->GetPage("select * from orderform where CreatTime like '%$time%' and State=1 and city='$live_city'",15);
				}elseif($check=='todaytiqu'){  //今日提货
			$time=date("Y-m-d");
			$dopage->GetPage("select * from orderform where TakeTime like '%$time%' and State=8 and city='$live_city'",15);
				}
		  }elseif($keyword!=""){
		  $dopage->GetPage("SELECT * FROM `orderform` where city='$live_city' and OrderId like '%$keyword%' or tiquma like '%$keyword%' or UserId like '%$keyword%'",15);		
		  }else{
		     $dopage->GetPage("SELECT * FROM `orderform` where city='$live_city'",15);	
		  }  
			  }
	  ...
	  //state订单状态(1, 待提取 2，返利单  3换购单 4，已失效 5，待付款 6待评价 7 以退款 8以提取 )
		...
		<tr align="left" class="dataTr" >
			...
			if($row['dingdantype']==2 && $row['kd_state']==1){
				echo "快递已发件，短信已发送";
			}else{
		  ...
		  <td align="center"><?php  if(!empty($rows)) echo $rows['Account']; ?></td>
		   ...
		   echo "&nbsp;&nbsp;预约时间：".$row['time'];
		   }?>" ><a href="javascript:vod('<?php echo $row['dingdantype'];?>','<?php echo $row['UserId'];?>','<?php echo $row['OrderId'];?>')"><?php
       ...
       <div id="jsddm"><a style="width: 24px;" title="删除" href="order_save.php?action=del2&id=<?php echo $row['Id']; ?>" onclick="return ConfDel(0);"><i class="fa fa-trash" aria-hidden="true"></i></a></div>
...
?>
<div class="bottomToolbar"><span class="selArea"><span>选择：</span> <a href="javascript:CheckAll(true);">全部</a> - <a href="javascript:CheckAll(false);">无</a> - <a href="javascript:DelAllNone('order_save.php');" onclick="return ConfDelAll(0);">删除</a></span></div>
...
<div class="quickToolbar">
	<div class="qiuckWarp">
		<div class="quickArea"><span class="selArea"><span>选择：</span> <a href="javascript:CheckAll(true);">全部</a> - <a href="javascript:CheckAll(false);">无</a> - <a href="javascript:DelAllNone('order_save.php');" onclick="return ConfDelAll(0);">删除</a></span> <span class="pageSmall"> <?php echo $dopage->GetList(); ?> </span></div>
...
<form name="form" id="form" method="post" action="order_save.php">
	<table width="100%" border="0" cellpadding="0" cellspacing="0" class="dataTable">
		...
		$dosql->Execute("SELECT * FROM `orderform` where address='$commercialSite'",$two);
       ...
       $dosql->Execute("SELECT Recommand FROM `commercialuser` where username='$username'",$three);
		...
		while($row = $dosql->GetArray($four)){
			...
			//echo $userId;
			$dosql->Execute("SELECT * FROM `orderform` where UserId='$userId'",$five);
			...
			for($i=$start;$i<count($newarr);$i++)
		   {
		   	$orderid=$newarr[$i];
			if($orderid!=""){
			$row=$dosql->GetOne("SELECT * FROM `orderform` where OrderId='$orderid'");
	  //state订单状态(1, 待提取 2，返利单  3换购单 4，已失效 5，待付款 6待评价 7 以退款 8以提取 )
			switch($row['State'])
			{
				case 1:
		...
		<tr align="left" class="dataTr" >
		  ...
		  <td align="center"><?php  if(!empty($rows)) echo $rows['Account']; ?></td>
           ...
           echo $r['CommercialName']; 
		   echo "&nbsp;&nbsp;预约时间：".$row['time'];
		   }?>"><a href="javascript:vod('<?php echo $row['dingdantype'];?>','<?php echo $row['UserId'];?>')"><?php
		     if(($row['address'])!=""){echo $row['address'];
			 }elseif(($row['dingdantype'])==2){
				 echo "<font color='#33F'><b>订单详情</B></font>";
				 }else{
