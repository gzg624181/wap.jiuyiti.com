...
$id=$s['Id']; //商户id
?>
<div class="topToolbar"><span class="title">系统默认优惠券：</span> &nbsp;&nbsp;&nbsp;
<?php 
if($s['defaults']==1){
?>
<span class="title"><a onclick="{if(confirm('确定更新默认优惠券吗?')){this.document.formname.submit();return true;}return false;}" href="quan_save.php?action=quan_update&id=<?php echo $id;?>&Commercial=<?php echo $Commercial;?>&usetime=<?php echo date("Y-m-d h:i:s",time());?>"><font color="red">更新到系统最新默认优惠券</font></a></span>
<?php }?>
...
$dosql->Execute("select * from coupons where Commodityid='$id' and type=1");
...
<form name="form" id="form" method="post" action="product_save.php">
	<table width="100%" border="0" align="center" cellpadding="0" cellspacing="0" class="dataTable">
		<tr align="center" class="head">
			...
			<td width="17%" align="left">商家名称</td>
			<td width="14%" align="center">商家账号</td>
			<td width="17%" align="center">消费额度范围</td>
			<td width="13%" align="center">金额</td>
			<td width="17%" align="center">优惠券生成日期</td>
			<td width="12%" align="center">是否启用</td>
		  <td width="3%" align="center">操作</td>
		</tr>
		<?php
		
		$dosql->Execute("SELECT * FROM coupons a inner join commercialuser b on a.Commodityid=b.Id where b.Commercial='$Commercial' and a.type=1");
   ...
   <input type="hidden" id="Id" value="<?php echo $row['Id'];?>" />
     <img  width="70" height="70" layer-src="../<?php echo $row['logo']; ?>" style="cursor:pointer; border-radius:40px; padding:3px;" onclick="message('<?php echo $row['Id']; ?>');"  src="../<?php echo $row['logo']; ?>" alt="<?php echo $row['CommercialName']; ?>" />
       </div></td>
