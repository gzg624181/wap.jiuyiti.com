...
<?php
$s=$dosql->GetOne("select * from commercialuser where Commercial='$userid'");
...
if($kd_state==0){
?>
<form name="form" id="form" method="post" action="order_save_sh.php" onsubmit="return quan();">
<table width="100%" border="0" cellspacing="0" cellpadding="0" class="formTable">
		<tr>
		  <td height="40" align="center">收货人：</td>
		  <td width="74%"><?php echo $s['kd_name'];?></td>
    </tr>
	<tr>
	...
	while($row=$dosql->GetArray()){
		if(is_array($row)){
		$price[]=$row['Quantity']*$row['shprice'];
...
<table width="100%" border="0" cellspacing="0" cellpadding="0" class="formTable">
  ...
  <tr>
    <td height="43"><?php  echo $row['shprice'];?>元，
  ...
  <tr>
    <td height="40" align="center">快递费用：</td>
    <td>¥      <span class="num" style="color:red;"><?php $kdprice=$r['PayKdMoney']; echo $kdprice;?></span></td>
  </tr>
  <tr>
  ...
  <tr>
    <td height="40" align="center">总计费用：</td>
    <td>¥ <span class="num" style="color:red;"><?php echo $kdprice+$jiuqian_price;?></span></td>
  </tr>
  <tr>
  ...
  <tr>
	...
	<div class="formSubBtn" style="float:left; margin-left: 187px;margin-top: 9px;">
      <input style="width:283px;height: 34px;line-height: 30px;margin-left: -4px;" class="layui-btn layui-btn-normal"  onclick="{if(confirm('确认要发送快递吗?')){this.document.formname.submit();return true;}return false;}" type="submit" value="确认发送快递,寄件短信" />

        <input type="hidden" name="action" id="action" value="kd_state_sh" />
        ...
        <input type="hidden" name="alias" id="alias" value="<?php echo $userid;?>" />
        <input type="hidden" name="creattime" id="creattime" value="<?php  echo $r['CreatTime']; ?>" />
        <input type="hidden" name="kd_phone" id="kd_phone" value="<?php echo $s['kd_phone'];?>" />
  </div>
</form>
<?php }else{ ?>
<table width="100%" border="0" cellspacing="0" cellpadding="0" class="formTable">
...
<table width="100%" border="0" cellspacing="0" cellpadding="0" class="formTable">
  ...
  <tr>
	...
	<input type="button" style="height: 34px;line-height: 30px; width：110px;" class="layui-btn layui-btn-normal" value="发件短信已发" />
  </div>
  <div id="pro"></div>
<?php }?>
</body>
