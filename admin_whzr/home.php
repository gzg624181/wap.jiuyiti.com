...
<table width="100%" style="background-image:url(templates/images/bg1.jpg); height:200px; margin-bottom:25px;border-radius: 3px;" >
  <tr>
    <td width="27%" height="53" class="fon" style="text-align: center">今日订单</td>
    <td width="29%" class="fon" style="text-align: center">今日提货</td>
    ...
    <td width="27%" height="32" class="ziti" style="text-align: center;"><a style=" color:white;" href="allorder.php?check=today"><?php
					$time=date("Y-m-d");
					$dosql->Execute("select * from orderform where CreatTime like '%$time%' and (State=1 or State=8)");
					$num=$dosql->GetTotalRow();
					echo $num;
					?></a></td>
    <td class="ziti" style="text-align: center"><a style="color:white;"href="allorder.php?check=todaytiqu"><?php
					$time=date("Y-m-d");
    ...
    <td class="ziti" style="text-align: center"><a style="color:white;" href="money.php?check=shenqintixian"><?php
					$time=date("Y-m-d");
					$dosql->Execute("select * from pickupmoney where State=0");
					$num=$dosql->GetTotalRow();
					if($num==0){
					echo $num;
						}else{
  ...
  <tr>
    <td width="27%" height="20" class="tor" style="text-align: center">昨日：<a style="color:white;" href="allorder.php?check=tomorrowdingdan"><?php
					...
					echo $num;
					?></a></td>
    <td style="text-align: center"><span class="tor">昨日：<a style="color:white;" href="allorder.php?check=tomorrowtiqu"><?php
					$time=date("Y-m-d",strtotime("-1 day"));
					$dosql->Execute("select * from orderform where TakeTime like '%$time%' and State=8");
					$num=$dosql->GetTotalRow();
					echo $num;
...
<div class="homeTeam" style="margin-bottom: 15px;">
