...
//获取推荐人的电话号码
function tel($Yaoqingma,$classes){
	global $Yaoqingma,$classes,$dosql;
	if($classes!="" && $classes==1){  //当推荐人是商户的时候
			$show=$dosql->GetOne("select * from commercialuser where Recommand='$Yaoqingma'");
			if(is_array($show)){
			$Phone=$show['Phone'];
			return  $Phone;
			}
			}
			if($classes!="" && $classes==0){  //当推荐人是会员的时候
			$show=$dosql->GetOne("select * from memberuser where Recommand='$Yaoqingma'");
			if(is_array($show)){
...
//获取推荐人的名称
function tjr($Yaoqingma,$classes){
	global $Yaoqingma,$classes,$dosql;
	if($classes!="" && $classes==1){  //当推荐人是商户的时候
			$show=$dosql->GetOne("select * from commercialuser where Recommand='$Yaoqingma'");
			if(is_array($show)){
			$CommercialName=$show['CommercialName'];
