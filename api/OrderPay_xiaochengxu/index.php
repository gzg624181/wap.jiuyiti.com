...
if($sum_jiuqian==$sum_prices){
 ...
 if(floatval($payamount)==0){    //酒钱支付
	 $paymenttype="1";
 }elseif(floatval($jiuqian)==0){ //微信支付
	 $paymenttype="2";
 }else{                          //混合支付
	 $paymenttype="3";
 }
...
/*	$dosql->Execute("select * from ordercommodity a inner join commodity b on a.CommodityId=b.Id where a.OrderId='$orderid'");
   ...
   $sql ="UPDATE `orderform` SET PayAmount='$payamount', State='1', PaymentType='$paymenttype',PayJiuQian='$jiuqian',paytype='$paytype' WHERE OrderId='$orderid'";
   $dosql->ExecNoneQuery($sql);

   //支付成功之后，通过微信向微信用户发送支付成功模板消息
