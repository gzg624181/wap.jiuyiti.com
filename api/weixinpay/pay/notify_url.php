...
class PayNotifyCallBack extends WxPayNotify
{
    ...
    //更改商户订单的状态改为 1 待提取
    $strsql="UPDATE `orderform_commercial` SET State='1' WHERE OrderId='$orderid'";
    // 执行sql查询
...
$jiuqian=$ref['PayJiuQian'];    //支付的酒钱
 if($jiuqian>0){   //酒钱支付
    ...
    $sqlg="UPDATE `orderform_commercial` SET State='1', PaymentType='$paymenttype',paytype=1,dingdantype=2 WHERE OrderId='$orderid'";
    // 执行sql查询
...
if($jqs != 0){
$sqlh = "select * from commercialuser where Id='$mid'";  //Sql语句
$go = mysql_fetch_assoc(mysql_db_query($mysql_database, $sqlh, $conn));
$jq=floatval($go['JiuQian']) - $jqs;
// echo $jq;
$sqli="UPDATE `commercialuser` SET JiuQian='$jq'  WHERE Id='$mid'";
    // 执行sql查询
