...
{
	function GetCache($date,$cachename)
	{
	$DOCUMENT_ROOT = $_SERVER['DOCUMENT_ROOT'];  //获取当前的文件夹路径
	
    $path=$DOCUMENT_ROOT."/cache/";              //缓存文件存放地址
	
    $msg= serialize($date);                      //将获取的数据序列化
	
    if (!is_dir($path)){
    mkdir($path,0777);  // 创建文件夹cache,并给777的权限（所有权限）
     } 
    $file = $path.$cachename.".txt";
...
{
	function DelCache($Idd)
	{
	$DOCUMENT_ROOT = $_SERVER['DOCUMENT_ROOT'];               //获取当前的文件夹路径
	
    $path=$DOCUMENT_ROOT."/cache/".$Idd.".txt";              //缓存文件存放地址
	
	if(file_exists($path)){
    $result = @unlink ($path);                               //删除缓存文件
...
function get_url($get_name,$classtype)
  { 
    global  $cfg_webpath;
	$arr = file_get_contents(stripslashes($cfg_webpath."api/".$get_name."/index.php?classtype=".$classtype));  //去除对象里面的斜杠
   ...
   function get_gd($get_name)
  {
	global  $cfg_webpath;
	$arr = file_get_contents(stripslashes($cfg_webpath."api/".$get_name."/index.php"));  //去除对象里面的斜杠
    $srr = json_decode($arr,true);
	if($srr['State']==1)
	{
	$str=$srr['Data'];  
   ...
   function get_productshow($get_name,$id)
  {
	global  $cfg_webpath;
	$arr = file_get_contents(stripslashes("https://wap.jiuyiti.zrcase.com/api/".$get_name."/index.php?id=".$id));  //去除对象里面的斜杠
    $srr = json_decode($arr,true);
	if($srr['State']==1)
	{
	$str=$srr['Data'];  
  ...
  function get_productsearch($get_name,$keywords)
  {
	global  $cfg_webpath;
	$arr = file_get_contents(stripslashes($cfg_webpath."api/".$get_name."/index.php?keywords=".$keywords));  //去除对象里面的斜杠
    $srr = json_decode($arr,true);
	if($srr['State']==1)
	{
	$str=$srr['Data'];  
  ...
  function get_shoppingcart($get_name,$userid)
  {
	global  $cfg_webpath;
	$arr = file_get_contents(stripslashes($cfg_webpath."api/".$get_name."/index.php?userid=".$userid));  //去除对象里面的斜杠
    $srr = json_decode($arr,true);
	if($srr['State']==1)
	{
	$str=$srr['Data'];  
  ...
  function del_shoppingcart($get_name,$id)
  {
	global  $cfg_webpath;
	$arr = file_get_contents(stripslashes($cfg_webpath."api/".$get_name."/index.php?id=".$id));  //去除对象里面的斜杠
    $srr = json_decode($arr,true);
	if($srr['State']==1)
	{
	$str=$srr['Data'];  
  ...
  function get_orderList_Client($get_name,$type,$userid)
  {
	global  $cfg_webpath;
	$arr = file_get_contents(stripslashes($cfg_webpath."api/".$get_name."/index.php?type=".$type."&userid=".$userid));  //去除对象里面的斜杠
    $srr = json_decode($arr,true);
	if($srr['State']==1)
	{
	$str=$srr['Data'];  
  ...
  function GetCommercialUser($get_name,$commercial)
  {
	 global  $cfg_webpath;
	$arr = file_get_contents(stripslashes($cfg_webpath."api/".$get_name."/index.php?commercial=".$commercial));  //去除对象里面的斜杠
    $srr = json_decode($arr,true);
	if($srr['State']==1)
	{
	$str=$srr['Data'];  
  ...
  function GetorderList_Client_Pc($get_name,$type,$userid)
  {
	  global  $cfg_webpath;
	$arr = file_get_contents(stripslashes($cfg_webpath."api/".$get_name."/index.php?type=".$type."&userid=
	".$userid));  //去除对象里面的斜杠
    $srr = json_decode($arr,true);
	if($srr['State']==1)
	{
 ...
 function get_typenama($typename)
 {
  switch($typename)
			{
				case '1': 
