...
function GetHeader($sid=1,$cid=0,$id=0,$str='')
{
	...
	if($str != '')
	{
		$header_str  = '<title>'.$str.'_'.$cfg_webname.'</title>'."\n";
	...
	else
	{
		...
		if(!empty($cid) && !empty($id))
		{
			...
			if(isset($r['infotype']))
			{
				...
				if(isset($r2['title']))
					$header_str .= $r2['title'].'_';

				if(isset($r['classname']))
					$header_str .= $r['classname'];

				$header_str .= '_'.$cfg_webname.'</title>'."\n";
				$header_str .= '<meta name="generator" content="'.$cfg_generator.'" />'."\n";
				$header_str .= '<meta name="author" content="'.$cfg_author.'" />'."\n";
		...
		else if(!empty($cid))
		{
			...
			if(!empty($r['seotitle']))
				...
				$header_str .= $r['classname'].'_'.$cfg_webname;
			else
		...
		{
			if($cfg_seotitle != '')
				$header_title = $cfg_seotitle. '_' .$cfg_webname;
			else
...
function GetHeader_product($sid=1,$cid=0,$id=0,$str='')
{
	global $dosql, $cfg_webname, $cfg_generator, $cfg_author,
	       $cfg_seotitle, $cfg_keyword, $cfg_description;
